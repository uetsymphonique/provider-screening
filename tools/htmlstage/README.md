...
HTML-staging tool: fetch a URL, **stage** the raw content (HTML/Markdown)
plus the extracted text into `artifacts/`, and append a line to
...
This is the sibling of `tools/pdfstage/main.py`: same manifest schema, same
`artifacts/` dir, differing only in `"kind": "html"` vs `"pdf"` -- one
grounding-check script can walk both kinds.

`main.py` deliberately does **not** go through WebFetch (used by
...
```
tools/htmlstage/
    main.py             CLI entrypoint: run the fetch chain, extract, stage, write manifest
    core/
...
`core/fetchers/` is split by **dependency weight**, not one file per method:
...
Chain order (see `main.py`'s module docstring for the full
rationale):
...
venv/Scripts/python.exe tools/htmlstage/main.py <url> \
...
venv/Scripts/python.exe tools/htmlstage/main.py <url> --out-dir <dir>
```
...
| Flag | Meaning |
|---|---|
| `--product` | Stage into `<domain>/runs/<product>/artifacts/` + append to the manifest (the same file `tools/pdfstage/main.py` writes to). Preferred. |
...
<domain>/runs/<product_id>/artifacts/
    ...
    manifest.jsonl                                                append-only, shared with tools/pdfstage/main.py
```
...
`core/fetchers/base.py` exposes `REGISTRY: dict[str, callable]` plus a
`@register("name")` decorator. A method that only needs a URL (no
CLI-specific kwargs) registers itself and gets dispatched by
`main.py._run_method` via `REGISTRY[method](url)` -- no change
needed to `main.py`. A method that needs an extra kwarg from the
CLI (like `direct`, `wayback`, `cc`) is wired explicitly in `_run_method`
instead (see `core/fetchers/__init__.py`'s docstring for why it isn't forced
through the registry).

Adding a new URL-only method:
1. Create a new module in `core/fetchers/` (or add to an existing one at the
...
3. Export it from `core/fetchers/__init__.py`.
4. Add `"xxx"` to `AUTO_CHAIN` in `main.py`, positioned by cost/
