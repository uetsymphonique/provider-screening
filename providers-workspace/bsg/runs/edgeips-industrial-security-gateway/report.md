...
**Evidence quality:** 8 items backed by ≥ 2 source_types; 13 items backed by vendor_doc only (confidence capped at medium per validator rule).
...
|-----|-------------|:-------:|:----:|-------|----------|
| 1.1 | Kiến trúc Tách biệt Phiên (Protocol Break): Chấm dứt phiên TCP/IP tại phễu ranh giới, ngắt hoàn toàn IP routing. | Not Supported | medium | - | TXOne documents EdgeIPS as a "transparent inline" IPS deployed "without IP changes or network redesign" and its FAQ states "EdgeIPS is a transparent device, not a switch" -- a bridging architecture that passes traffic through unmodified, which contradicts a protocol-break design that terminates TCP/IP sessions at the boundary. [1], [9] |
| 1.2 | Cách ly Phần cứng (Hardware Isolation): Thiết kế 2 bo mạch xử lý tách biệt kết nối qua FPGA hoặc Shared Memory cách ly. | Unknown | low | - | no evidence found (The documented fail-safe is a Gen3 hardware bypass on each port pair; no dual-processing-board hardware isolation design (FPGA or isolated shared memory) is documented.) |
...
| ID  | Requirement | Verdict | Conf | Value | Evidence |
|-----|-------------|:-------:|:----:|-------|----------|
| 2.1 | Giải phẫu & Tái tạo Nội dung (CDR): Bóc tách và tái tạo 100% các định dạng Office (DOCX, XLSX), PDF, Image, CAD. | Unknown | low | - | no evidence found (L2-L7 deep packet inspection and antivirus scanning of live traffic are documented; no file-level CDR (Office/PDF/Image/CAD reconstruction) capability is documented.) |
...
| 2.5 | Kiểm soát Luồng Thông tin (IFC): Lọc dữ liệu dựa trên Nhãn An ninh (Security Labels) gắn kèm tập tin. | Unknown | low | - | no evidence found (OT protocol allowlist/blocklist filtering is documented; no security-label-based information flow control on files is documented.) |
