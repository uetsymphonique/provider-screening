...
Global summary + total cost dumped to `providers-workspace/microsegmentation/runs/_batch/summary-<ts>.json`
(or `summary-pi-<ts>.json` for the `pi` subcommand).

`--domain` and `--mode` are required for `run_batch.py`, along with the agent
subcommand (`claude` or `pi`); `tools/pdfstage/main.py` / `tools/htmlstage/main.py --domain`
