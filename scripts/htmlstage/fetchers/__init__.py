"""Fetch methods for html_to_text.py's auto-fallback chain: direct HTTP,
...
Package layout (grouped by dependency weight, not one file per method):
    base.py     -- FetchError, the charset-sniffing _decode() helper, and
                   ...
                   html_to_text.py dispatch by name instead of growing an
...
the function, and adding the name to AUTO_CHAIN in html_to_text.py -- no
changes to html_to_text.py's dispatch logic. Methods that also need
CLI-specific override kwargs (direct's browser_ua, wayback's timestamp,
cc's index/status) stay wired explicitly in html_to_text.py._run_method
...
(proxy_url, cc_index, ...). Fetchers never write to disk -- html_to_text.py
...
driver in html_to_text.py can catch and escalate to the next method instead
