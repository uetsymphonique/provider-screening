...
Purpose:
  ...
  This is the HTML sibling of `pdf_to_text.py`: it fetches the page (no
  ...
  pdf_to_text.py (distinguished by "kind": "html" vs "pdf"), so one
...
Fetch methods (--method, default "auto"):
  ...
  somehow" with an identical staging/manifest contract:
    direct     -- HTTP GET impersonating a real browser's TLS/JA3
                  ...
                  (scripts/htmlstage/fetchers.py:fetch_direct)
  ...
  htmlstage.extract.MIN_REAL_CONTENT_CHARS (250) chars, OR the extracted
  text matches a known bot-challenge banner (htmlstage.extract.
...
Extraction engine:
  ...
  (htmlstage.extract._TextExtractor) so a page never silently yields 0 chars
...
Usage:
  python scripts/htmlstage/html_to_text.py <url> --product <product_id> [--domain bsg]
  python scripts/htmlstage/html_to_text.py <url> --method proxy-html --product <id>
  python scripts/htmlstage/html_to_text.py <url> [--out-dir <dir>] [--slug <name>]
                                                   [--preview <N>]
...
from htmlstage.extract import MIN_REAL_CONTENT_CHARS, detect_interstitial, extract_text  # noqa: E402
from htmlstage.fetchers import (  # noqa: E402
    REGISTRY,
    FetchError,
    fetch_common_crawl,
    fetch_direct,
...
from htmlstage.staging import append_manifest, resolve_staging, sha256_bytes, slugify  # noqa: E402
from domains import DOMAINS, REPO_ROOT  # noqa: E402
...
def _run_method(method: str, url: str, args: argparse.Namespace) -> tuple[bytes, str, dict]:
    ...
    # explicitly rather than through htmlstage.fetchers.REGISTRY -- see
    # htmlstage/fetchers/__init__.py's docstring for the split rationale.
    if method == "direct":
