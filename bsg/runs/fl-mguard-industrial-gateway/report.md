...
**Evidence quality:** 12 items backed by ≥ 2 source_types; 1 items backed by vendor_doc only (confidence capped at medium per validator rule).

**Not-applicable items:**
- **1.1:** Vendor documents the FL MGUARD 2102 as a 'Simple stateful inspection firewall' security router; an independent tool description calls the mGuard series 'a family of firewall/router devices'. A firewall/router forwards traffic at the network layer rather than terminating sessions in a protocol-break architecture.
- **1.2:** The FL MGUARD 2102 is documented as a single stand-alone DIN-rail security router with a stateful inspection firewall; no dual processing-board or FPGA/shared-memory isolation design is described for this firewall-class product.
- **1.5:** FL MGUARD is documented as a stateful-inspection firewall/security router rather than a guard with an internal control core that signs clean data before re-initiating sessions; no internal data stamping is described.
- **2.1:** The product class is a firewall/security router; content disarm and reconstruction of Office, PDF, Image or CAD files is not part of the documented function set.
- **2.4:** The FL MGUARD 4305 is documented as a configurable stateful inspection firewall filtering MAC/IP addresses, ports and protocols; W3C-schema validation of XML/JSON/FIXM/AIXM structures is a CDS function not documented for this firewall class.
- **2.5:** FL MGUARD is a firewall/security router; information flow control based on security labels attached to files is not part of the documented function set for this product class.
- **2.7:** FL MGUARD is a firewall/security router; an anti-steganography engine for PNG/JPEG/BMP payloads is not part of the documented function set for this product class.
...
| 1.2 | Cách ly Phần cứng (Hardware Isolation): Thiết kế 2 bo mạch xử lý tách biệt kết nối qua FPGA hoặc Shared Memory cách ly. | N/A | medium | — | The FL MGUARD 2102 is documented as a single stand-alone DIN-rail security router with a stateful inspection firewall; no dual processing-board or FPGA/shared-memory isolation design is described for this firewall-class product. [4], [9] |
...
| 1.5 | Chữ ký số Nội bộ (Internal Data Stamping): Lõi kiểm soát ký số dữ liệu sạch trước khi cho phép phễu nội khởi tạo phiên mới. | N/A | medium | — | FL MGUARD is documented as a stateful-inspection firewall/security router rather than a guard with an internal control core that signs clean data before re-initiating sessions; no internal data stamping is described. [4], [9] |

### Category 2 — Inspection & CDR Engine

| ID  | Requirement | Verdict | Conf | Value | Evidence |
|-----|-------------|:-------:|:----:|-------|----------|
| 2.1 | Giải phẫu & Tái tạo Nội dung (CDR): Bóc tách và tái tạo 100% các định dạng Office (DOCX, XLSX), PDF, Image, CAD. | N/A | medium | — | The product class is a firewall/security router; content disarm and reconstruction of Office, PDF, Image or CAD files is not part of the documented function set. [4], [9] |
...
| 2.4 | Phân tích Cú pháp Schema (Schema Check): Kiểm tra tính hợp lệ của cấu trúc XML, JSON, FIXM, AIXM theo W3C Schema. | N/A | medium | — | The FL MGUARD 4305 is documented as a configurable stateful inspection firewall filtering MAC/IP addresses, ports and protocols; W3C-schema validation of XML/JSON/FIXM/AIXM structures is a CDS function not documented for this firewall class. [5], [9] |
| 2.5 | Kiểm soát Luồng Thông tin (IFC): Lọc dữ liệu dựa trên Nhãn An ninh (Security Labels) gắn kèm tập tin. | N/A | medium | — | FL MGUARD is a firewall/security router; information flow control based on security labels attached to files is not part of the documented function set for this product class. [4], [9] |
...
| 2.7 | Anti-Steganography Engine: Phát hiện và loại bỏ dữ liệu ẩn giấu bên trong file hình ảnh (PNG, JPEG, BMP). | N/A | medium | — | FL MGUARD is a firewall/security router; an anti-steganography engine for PNG/JPEG/BMP payloads is not part of the documented function set for this product class. [4], [9] |
