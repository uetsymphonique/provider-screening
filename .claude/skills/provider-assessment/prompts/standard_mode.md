...
providers-workspace/{DOMAIN}/GUIDE.md - it OVERRIDES the
...
2. OUTPUT LOCATION
   ...
   Do NOT hand-write report.md. After assessment.json validates (step 8), run:
     venv/Scripts/python.exe \
       .claude/skills/provider-assessment/scripts/render_report.py \
       --domain {DOMAIN} \
...
4. SOURCE STAGING (mandatory for EVERY cited source - PDF or web page)
   ...
   PDF source:
     venv/Scripts/python.exe tools/pdfstage/main.py <pdf-url> \
       --product {product_id} --domain {DOMAIN}
   Web page source:
     venv/Scripts/python.exe tools/htmlstage/main.py <page-url> \
       --product {product_id} --domain {DOMAIN}
   ...
   Citation etiquette:
     - sources.jsonl.raw_url  → the ORIGINAL URL (PDF or page), not the local
                                 ...
                                 passed to tools/pdfstage/main.py / tools/htmlstage/main.py
...
8. VALIDATE
   After generating everything, run:
     venv/Scripts/python.exe \
       .claude/skills/provider-assessment/scripts/validate_assessment.py \
       --domain {DOMAIN} \
       providers-workspace/{DOMAIN}/runs/{product_id}/assessment.json \
   ...
   quote is real, only that its IDs exist - this does):
     venv/Scripts/python.exe \
       .claude/skills/provider-assessment/scripts/verify_citation_grounding.py \
